        ...
        def exporter_csv(cnx, list_table_a_exporter):
            for table in list_table_a_exporter:
                ...
                with open("table/tout.csv", 'a', newline='', encoding='utf-8') as file:
                    ...
                    print(text("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'"+ table +"'  AND TABLE_SCHEMA = 'blandeau_gestlab19';"))
                    ...
                    for att in attribut:
                        writer.writerow([f"*{att}"])  # Écrire les lignes commençant par "*"
