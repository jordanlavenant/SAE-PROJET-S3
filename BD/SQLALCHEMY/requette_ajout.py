...
def get_all_information_to_Materiel(cnx, nomcat=None):
    my_list = []
    if nomcat is None:
        result = cnx.execute(text("select idMateriel, nomMateriel, idCategorie,nomCategorie, idDomaine,nomDomaine,quantiteLaboratoire  from MATERIEL natural join STOCKLABORATOIRE natural join DATEPEREMPTION natural join DOMAINE natural join CATEGORIE natural join FDS;"))
        for row in result:
            my_list.append((row[0],row[1],row[2],row[3],row[4],row[5],row[6]))
    else:
        result = cnx.execute(text("select idMateriel, nomMateriel, idCategorie,nomCategorie, idDomaine,nomDomaine,quantiteLaboratoire  from MATERIEL natural join STOCKLABORATOIRE natural join DATEPEREMPTION natural join DOMAINE natural join CATEGORIE natural join FDS where nomCategorie = '" + nomcat + "';"))
        for row in result:
            my_list.append((row[0],row[1],row[2],row[3],row[4],row[5],row[6]))
    print(my_list)
    return my_list
